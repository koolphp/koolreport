...
class ResourceManager extends Base
{
     ...
     * @param string $version Version of the asset folder, normally it is the version of widget
	...
	public function publishAssetFolder($fullLocalPath,$version="")
	{
        ...
        if(!$assets)
		{
            ...
            if($assetUrl==$fullLocalPath)
            {
                ...
                $script_folder = Utility::standardizePathSeparator(realpath(dirname($_SERVER["SCRIPT_FILENAME"])));
                $asset_path = $script_folder."/koolreport_assets";            
                $asset_url = Utility::str_replace_first($document_root,"",$script_folder)."/koolreport_assets";
		...
		if($assets)
		{
			...
			$reportClassFolder = Utility::standardizePathSeparator(dirname(Utility::getClassPath($this->report)));
			if(strpos($targetAssetPath,"/")!== 0 && is_dir($reportClassFolder."/".$targetAssetPath))
			{
				//Check if relative targetAssetPath existed
				$targetAssetPath = Utility::standardizePathSeparator(realpath($reportClassFolder."/".$targetAssetPath));
			}
			else if(is_dir($targetAssetPath))
			{
				//Check if full targetAssetPath existed
				$targetAssetPath = Utility::standardizePathSeparator(realpath($targetAssetPath));
			}
			else
			{
			...
			$objectHashFolderName = crc32("koolreport".$fullLocalPath.@filemtime($fullLocalPath).$this->report->version().$version);
     ...
     * @param string $src Url link to javascript file
     * @param array $options Additional options for script tag
     * @param string $at The location of the script whether at "begin" of report or at "end" of report
     * @return ResourceManager this resource manager object
     */
     ...
     * @param string $script Javascript code we want to run
     * @param array $options Any additional options you want to put to script tag
     * @param string $at The location you want to add the script to whether at "begin" of report or at the "end" of report
     * @return ResourceManager This resource manager object
     */
     ...
     * 
     * Beside css, there could be another type of resources you want to add via link tag
     * 
