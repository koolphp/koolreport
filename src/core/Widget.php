...
class Widget extends Base
{
	...
	/**
	 * @var AssetManager $assetManager The widget assetManager to help manage widget resource
	 * public widget client folder
	 */
	...
	/**
	 * @var string $onReady Set the javascript in string to be execute after widget initiation
	 */
	protected $onReady;
	...
	/**
	 * @var bool $withoutLoader If true, widget will not render with KoolReport Loader
	 */
	...
	public function __construct($params=null)
	{
		...
		if($this->report===null && isset($GLOBALS["__ACTIVE_KOOLREPORT__"]) && $GLOBALS["__ACTIVE_KOOLREPORT__"]!==null)
		{
	 ...
	 * 
	 * This method is derived by sub class to provide information of resources settings
	 * such as javascript, css, library to be loaded
	 * 
	 * @param array The associate of resource settings
	 */
	 ...
	 * 
	 * The method will read the resource settings and work with report's resource manager
		...
		if($settings)
		{
			...
			if($library)
			{
				...
				foreach($library as $libName)
				{
					...
					{
						case "jquery":
							$publicAssetUrl = $this->getReport()->getResourceManager()->publishAssetFolder(realpath(dirname(__FILE__)."/../clients/jquery"));
						...
						case "font-awesome":
							...
							array_push($lib["css"],$publicAssetUrl."/css/font-awesome.min.css");
							break;
						break;
					}
				}
				...
				if($lib["js"]!=array())
				{
					$resources["js"] = array_merge($lib["js"],array($resources["js"]));
				}
				if($lib["css"]!=array())
				{
					$resources["css"] = array_merge($lib["css"],array($resources["css"]));
				}
			}	
		}
		return $resources;
	 ...
	 */
	protected function attachResourceToEnd(&$desination,$attachment)
	{
		for($i=count($desination)-1;$i>-1;$i--)
		{
			if(is_array($desination[$i]))
			{
				if($this->attachResourceToEnd($desination[$i],$attachment))
				{
					return true;
				}
			}
		}
		array_push($desination,$attachment);
		return true;
	}
	 ...
	 * 
	 * @return array Resource settings after converted
	 */
	 ...
	 * 
	 * The method will return name of widget set by user or set automatically by report
	 * 
	 ...
	 * 
	 * @param string $prefix The prefix to the widget name if the name is generated automatically.
	 */
	 ...
	 * DataStore
	 * 
	 * @param mixed Mixed source include array,collection, DataStore, DataSource, Process
	 * @return DataStore A DataStore containing data.
	 */
	protected function standadizeDataSource($dataSource)
	{
		...
		if($dataSource!==null)
		{
			if(is_callable($dataSource))
			{
				...
				$dataSource = call_user_func_array($dataSource,func_get_args());
			}
					...
					foreach($dataSource as $i=>$row)
					{
						...
						switch($rowType)
						{
							...
							case "array":
								if($i>0)
								{
									$finalDataSource->push(array_combine($keys_for_array,$row));
								}
								break;
						}
					}
	...
	protected function useDataSource()
	{
		...
		$this->dataStore = $this->standadizeDataSource($dataSource);
	}
	 ...
	 * 
	 * @param Node A node type include DataSource, Process or DataStore
	...
	protected function useLanguage()
	{
		...
		if($this->language!==null)
		{
			...
			{
				$languageFile = $this->getWidgetFolder()."/languages/".Utility::getClassName($this).".".strtolower($this->language).".json";
	 ...
	 * @param string The translated text for language
	 */
	...
	{
		return Utility::standardizePathSeparator(realpath(dirname(Utility::getClassPath($this))));
	}
	/**
	 ...
	 * 
	 * @param string $template The template name that will be used to render widget content.
	 * @param array $variables The variables to fill the template
	 * @param bool $return Whether template will render content or return the content
	 * @return string It could return html of widget if $return parameter is set to true
	 */
	 ...
	 * @param bool $return Set to true if you want to get html rather than rendering widget directly
